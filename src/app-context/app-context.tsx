...
const AppContextProvider: React.FC<React.PropsWithChildren> = (props) => {
    // Local Storage
    const [ cacheTime, setCacheTime ] = useChromeLocalState<string | undefined>("cacheTime", undefined);
    const [ transactions, setTransactions ] = useChromeLocalState<KoinlyTransaction[]>("transactions", []);
    const [ session, setSession ] = useChromeLocalState<KoinlySession | undefined>("session", undefined);

    // Sync Storage
    const [ csvSelections, setCSVSelections ] = useChromeSyncState<CSVSelections>("csvSelections", {});
